...
const Leaflet: React.FC<PropsFromRedux> = (props) => {
  ...
  const [midMarker, setMidMarker] = useState<{
    marker: Marker;
    popUp: Popup;
  }>();
  ...
  useEffect(() => {
    const myMap = new Map(
      mapContainerRef.current ? mapContainerRef.current : "map",
      {
  ...
  if (map) {
    ...
    for (let userMarker of userMarkers) {
      ...
      if (!props.users.map((user) => user._id).includes(userMarker.user._id)) {
        ...
        setUserMarkers(
          userMarkers.filter((um) => um.user._id !== userMarker.user._id),
        );
      }
    }
        ...
        for (let userMarker of userMarkers) {
          if (userMarker.user._id === user._id) {
            ...
            if (!user.checked) {
              userMarker.marker.remove();
              setUserMarkers(
                userMarkers.filter((m) => m.user._id !== user._id),
              );
              break;
            }
            foundMarker = true;
            userMarker.marker.setLatLng({
              lng: user.longitude,
              lat: user.latitude,
            });
            break;
    ...
    if (midMarker) {
      if (response) {
        // change marker
        midMarker.marker.setLatLng({
          lng: response.longitude,
          lat: response.latitude,
        });
        midMarker.popUp.setPopupContent(
          response.longitude + "\n" + response.latitude,
        );
      } else {
      ...
      if (response) {
        //create popup
        const popUp = new Popup().setPopupContent(
          response.longitude + "\n" + response.latitude,
        );
        //create marker
        const markerIcon = document.createElement("i");
        ...
        markerIcon.className =
          "marker middle-point fas fa-map-marker-alt fa-5x";
        const icon = new DivIcon({ html: markerIcon });
